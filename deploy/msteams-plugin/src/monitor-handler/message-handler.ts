...
// Stubs for functions added to plugin-sdk after this plugin version was published.
function isDangerousNameMatchingEnabled(_cfg: unknown): boolean {
  if (_cfg && typeof _cfg === "object" && "dangerousNameMatching" in _cfg) {
    return Boolean((_cfg as Record<string, unknown>).dangerousNameMatching);
  }
  return false;
}

function resolveDefaultGroupPolicy(cfg: unknown): string | undefined {
...
export function createMSTeamsMessageHandler(deps: MSTeamsMessageHandlerDeps) {
  ...
  const handleTeamsMessageNow = async (params: MSTeamsDebounceEntry) => {
    ...
    // --- Multi-tenant profile middleware ---
    try {
      const profileResult = await ensureUserProfile(senderId, senderName);
      if (profileResult.created) {
        log.info("created new profile", { userId: senderId, displayName: senderName });
      }
    } catch (profileErr) {
      log.debug?.("profile ensure failed, using global config", {
    ...
    if (cmdResult.handled) {
      if (cmdResult.reply) {
        try {
          await context.sendActivity(cmdResult.reply);
        } catch (replyErr) {
          log.debug?.("chat command reply failed", { error: String(replyErr) });
        }
      }
      return;
    }
    ...
    let perUserCfg = cfg;
    try {
      const agentConfig = await loadAgentConfig(senderId);
      const wsDir = profileWorkspaceDir(senderId);
      const sessDir = profileSessionsDir(senderId);
        ...
        agents: {
          ...
          defaults: {
            ...
            ...(agentConfig.model?.primary
              ? { model: { ...cfg.agents?.defaults?.model, primary: agentConfig.model.primary } }
              : {}),
          },
        },
        session: {
      ...
      log.debug?.("applied per-user config", {
        userId: senderId,
        workspace: wsDir,
        sessions: sessDir,
        model: agentConfig.model?.primary ?? "default",
      });
    } catch (cfgErr) {
      log.debug?.("per-user config failed, using global config", {
        error: String(cfgErr),
      });
    }
    // --- End multi-tenant profile middleware ---
    ...
    const mediaList = await resolveMSTeamsInboundMedia({
      ...
      log,
      preserveFilenames: (perUserCfg as { media?: { preserveFilenames?: boolean } }).media
        ?.preserveFilenames,
    });

    const mediaPayload = buildMSTeamsMediaPayload(mediaList);
    const envelopeFrom = isDirectMessage ? senderName : conversationType;
    const storePath = core.channel.session.resolveStorePath(perUserCfg.session?.store, {
      agentId: route.agentId,
    });
    const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(perUserCfg);
    ...
    try {
      ...
      try {
        if (await isOnboardingNeeded(senderId)) {
          await completeOnboarding(senderId);
          log.info("onboarding complete", { userId: senderId });
        }
      } catch {
        // Non-critical — onboarding marker will be cleared next time
      }
